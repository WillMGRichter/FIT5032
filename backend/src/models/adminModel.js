...
async function listUsers() {
  const { rows } = await pool.query(
    'SELECT id, email, first_name, last_name, firebase_uid, role, created_at FROM users ORDER BY created_at',
  )
...
async function updateUserRole(id, role) {
  const { rows } = await pool.query(
    'UPDATE users SET role = $2, updated_at = now() WHERE id = $1 RETURNING id, email, first_name, last_name, firebase_uid, role, created_at',
    [id, role],
  )
